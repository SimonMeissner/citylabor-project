...
links = list(map(''.join, zip((['https://gardenplanner.almanac.com'] * len(links)),
            [item for sublist in links for item in sublist])))
...
def scrap(address, climate):
    ...
    for i in range(len(links)):
        ...
        space[i] = driver.find_elements_by_id("ctl00_main_lblSpacingSingle")[0].text

        space[i] = space[i].split('(')[1].split('m)')[0]
        
        print(space[:i])

        time.sleep(10)

        driver.find_element(By.ID, "planting-times-btnStart").click()

        #driver.find_element_by_class_name("planting-times-button").click()

        time.sleep(2)

        if "flower" in driver.find_element_by_class_name("plant-detail").text :
              
           category[i] = "flower"

           print(category)

        elif "fruit" in driver.find_element_by_class_name("plant-detail").text :
        ...
        imgs[i] = driver.find_element_by_class_name("planting-times-plant-list-row").find_elements_by_class_name(
            "planting-times-month")

        for j in range(12):

            months[i][j] = int(imgs[i][j].find_element_by_tag_name('img').get_attribute('src').replace(
                "https://www.growveg.com/assets/images/plant-list-bars/", "").replace(".jpg", ""))
    ...
    for i in range(len(wtp)):
        ...
        if len(when_plant) == 0 or len(when_harvest) == 0:

            min_g_t.append(0)
            max_g_t.append(0)

        elif min(when_plant) == min(when_harvest):

            try:

                min_g_t.append(abs((min(when_plant) - when_harvest[1]) * 30))
        ...
        elif min(when_harvest) < min(when_plant):

            try:

                min_g_t.append((12 - min(when_plant) + when_harvest[1]) * 30)
                max_g_t.append((12 - min(when_plant) + max(when_harvest)) * 30)
...
#            size[i] = int(space[i].split("(")[1][:2]) * int(space[i].split("(")[2][:-2][:2])
 ...
 #       except:
  ...
  #          size[i] = (int(space[i].split("(")[1][:1]) * 100) * (int(space[i].split("(")[2][:-2][:1]))
    ...
    df = pd.DataFrame({'plant_name': names, 'links': links,
                       'required_space': space, 'when_to_plant': wtp_list, 'min_growing_time': min_g_t,
                       'max_growing_time': max_g_t, 'when_to_harvest': wth_list, 'climate': clim, 'category': category})
...
df_scrap = pd.concat([df01, df02, df03, df06, df07, df08, df09])
